...
class ZXTransformer:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, optimize: Optional[Callable[[zx.Circuit], zx.Circuit]] = None):
        ...
        Args:
            optimize: The optimization routine to execute. Defaults to
    ...
    def _cirq_to_circuits_and_ops(  # pylint: disable=too-many-locals
        self, circuit: circuits.AbstractCircuit
    ) -> List[Union[zx.Circuit, cirq.Operation]]:
        """Convert an AbstractCircuit to a list of PyZX Circuits and cirq.Operations. As much of
        the AbstractCircuit is converted to PyZX as possible, but some gates are not supported
        by PyZX and are left as cirq.Operations.
        ...
        num_measurements = sum(
            len(op.qubits)
            for moment in circuit for op in moment if cirq.is_measurement(op)
        )
        ...
        for moment in circuit:
            for op in moment:
                ...
                if isinstance(op.gate, cirq.MeasurementGate):
                    ...
                    for i, qubit in enumerate(op.qubits):
                        ...
                        _ensure_circuit().add_gate(
                            PyzxMeasurement(
                                self.qubit_to_index[qubit], result_bit=bit_index
                            )
                        )
                    continue
                ...
                if isinstance(op.gate, cirq.ResetChannel):
                    _ensure_circuit().add_gate(
                        PyzxReset(self.qubit_to_index[op.qubits[0]])
                    )
                    continue
    ...
    def _recover_circuit(  # pylint: disable=too-many-locals,too-many-statements
        self, circuits_and_ops: List[Union[zx.Circuit, cirq.Operation]]
    ) -> circuits.Circuit:
        """Recovers a cirq.Circuit from a list of PyZX Circuits and cirq.Operations.

        :param circuits_and_ops: The list of (optimized) PyZX Circuits and cirq.Operations from
                                 which to recover the cirq.Circuit.
            ...
            for gate in circuit_or_op.gates:
                ...
                if isinstance(gate, PyzxMeasurement):
                    if gate.result_bit is None:
                        raise ValueError(
                            "Invalid measurement data: missing result_bit."
                        )
                ...
                if isinstance(gate, PyzxReset):
                    cirq_circuit.append(
                        cirq.ResetChannel()(self.qubits[gate.target])
                    )
                    continue
                ...
                if isinstance(gate, ConditionalGate):
                    ...
                    qubit = self.qubits[getattr(inner, 'target')]
                    key_name = gate.condition_register
                    cirq_circuit.append(
                        cirq_gate(qubit).with_classical_controls(key_name)
                    )
                    continue
