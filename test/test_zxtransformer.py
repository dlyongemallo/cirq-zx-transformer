...
def _run_zxtransformer(
    qc: cirq.Circuit, optimize: Optional[Callable[[zx.Circuit], zx.Circuit]] = None
) -> None:
...
def _run_zxtransformer_nonunitary(
    ...
    """
    zx_qc = ZXTransformer(optimize)(qc)
    qubit_order = sorted(qc.all_qubits())
    original_dm = cirq.final_density_matrix(qc, qubit_order=qubit_order, ignore_measurement_results=True)
    transformed_dm = cirq.final_density_matrix(zx_qc, qubit_order=qubit_order, ignore_measurement_results=True)
...
def test_measurement_converted_to_pyzx() -> None:
    ...
    assert zxtransformer.measurement_keys == ['c']
...
def test_multi_qubit_measurement_converted_to_pyzx() -> None:
    ...
    assert len(circuits_and_ops) == 1
    assert isinstance(circuits_and_ops[0], zx.Circuit)
    assert zxtransformer.measurement_keys == ['m', 'm']
    pyzx_circuit = circuits_and_ops[0]
